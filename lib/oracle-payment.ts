...
import type { OracleFeed } from "./oracle-store";
...
export function paymentRequiredBody(feed: OracleFeed, resourceUrl: string) {
  const requirement = paymentRequirement(feed, resourceUrl);
  return {
...
export async function verifyOraclePayment(request: Request, feed: OracleFeed, resourceUrl: string) {
  const demoPayment = request.headers.get("x-demo-payment");
  const paymentHeader = request.headers.get("x-payment");
  const mode = process.env.ORACLE_PAYMENT_MODE || "demo";
  const requirement = paymentRequirement(feed, resourceUrl);
  ...
  if (demoPayment && mode !== "strict") {
    return {
      ...
      paymentResponse: encodePaymentResponse({ mode: "demo", feed: feed.slug }),
    };
  }
...
async function callFacilitator(url: string, body: unknown) {
  try {
    ...
    // x402 facilitators return HTTP 200 even when they reject a payment, so the
    // verdict must be read from the body. /verify uses isValid + invalidReason and
    // /settle uses success + errorReason. Fail closed on any negative signal.
    const ok =
      response.ok &&
      json.isValid !== false &&
      json.valid !== false &&
